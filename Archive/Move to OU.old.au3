...
#AutoIt3Wrapper_Res_Fileversion=2011.7.25.0
...
If Not FileExists ( "c:\windows\system32\dsget.exe" ) Then Exit
...
Local $msg, $PCName1, $PCName2, $Server
Local $line, $FQDN, $ButtonState, $OU, $FQOU, $moveit
Local $RID, $RID1, $Saying

Opt ( "GUICoordMode", 1 )

GUICreate ( "Move PC to OU", 1024 , 640 , Default , Default )
...
GUICtrlCreateLabel 						( "Enter PCN:", 300 , 42 ,  Default	, Default )
	$PCName1 = 		GUICtrlCreateInput 	( ""          , 360 , 38 ,      300 ,  20 )
GUIStartGroup ( )
...
;~ 			$Server = "CTYCTR-DC3"
		...
		If GUICtrlRead ( $radio_T1 ) = 1 Then
			If GUICtrlRead ( $radio_124 ) = 1 Then
				$Type = ""
			EndIf
		EndIf

		If GUICtrlRead ( $radio_T2 ) = 1 Then
			If GUICtrlRead ( $radio_124 ) = 1 Then
				$Type = ""
			EndIf
		EndIf
		...
		If GUICtrlRead ( $radio_T1 ) = 1 Then
			If GUICtrlRead ( $radio_123 ) = 1 Then
				$Type = ""
			EndIf
		EndIf
...
;~ 	The below is for OUs with no "Desktops" - OU=Remote Computers,OU=HeadStart,OU=Childrens Services,DC=hcbocc,DC=ad
		If GUICtrlRead ( $radio_T1 ) = 1 Then
...
;~ 		If $OU = "" Then ContinueLoop
		If $msg = $MoveButton Then
			$PCName2 = GUICtrlRead ( $PCName1 )
			If StringLen ( $PCName2 ) < 6 Then
				MsgBox ( 0 , $ScriptName , "PCN must contain at least 6 characters." )
				ContinueLoop
			EndIf
			If GUICtrlRead ( $radio_T1 ) = 4 Then
				If GUICtrlRead ( $radio_T2 ) = 4 Then
					MsgBox ( 0 , $ScriptName , "Please pick a type." )
					ContinueLoop
				EndIf
			EndIf
			If $OU = "" Then
				MsgBox ( 0 , $ScriptName , "Please pick an OU." )
				ContinueLoop
			EndIf


			$FQDN = Run ( @ComSpec & " /c dsquery.exe computer domainroot -name *" & $PCName2 & "* -s " & $Server & " -limit 1", "c:\windows\system32", @SW_HIDE , 2 )
			ProcessWaitClose ( $FQDN )
			While 1
				$line = StdoutRead ( $FQDN )
				If @error Then ExitLoop
				If Not $line = 0 Then ExitLoop
			WEnd
			...
			If $line = "" Then
				MsgBox ( 262208, "Well, crap!", "A variable turned up empty which means the DC does not know about the PC." & @CRLF & @CRLF & "It's possible that the PC was deleted from hcbocc.ad, or was never joined.       " & @CRLF & @CRLF & "Double check the PCN." )
				ContinueLoop
			EndIf

			$FQOU = StringFormat ( '"%s%s"', $Type, $OU )

			If MsgBox ( 262180, "Move the following?", $line & Chr ( 10 ) & "(" & $Server & ")" ) = 6 Then
;~ 				MsgBox ( 0 , "title" , $Server )
				$moveit = RunWait ( @ComSpec & " /c for /f ""delims=;"" %A in ('@dsquery.exe computer domainroot -name *" & $PCName2 & "* -s " & $Server & " -limit 1^') do dsmove.exe %A -newparent " & $FQOU & " -server " & $Server & "", "c:\windows\system32" , @SW_HIDE )
				$moveit = Run ( @ComSpec & " /c for /f ""delims=;"" %A in ('@dsquery.exe computer domainroot -name *" & $PCName2 & "* -s " & $Server & " -limit 1^') do for /f %B in ('@dsquery.exe server -o rdn^') do dsmove.exe %A -newparent " & $FQOU & " -server %B", "c:\windows\system32" , @SW_HIDE )
				...
				RunWait ( @ComSpec & " /c dsquery.exe computer domainroot -name *" & $PCName2 & "* -s " & $Server & " -limit 1|dsmod.exe computer -disabled no ", "c:\windows\system32", @SW_HIDE )
				If GUICtrlRead ( $radio_123 ) = 1 Then RunWait ( @ComSpec & " /c dsquery.exe computer domainroot -name *" & $PCName2 & "* -s " & $Server & " -limit 1|dsmod.exe computer -disabled yes ", "c:\windows\system32", @SW_HIDE )
				ProcessWaitClose ( $moveit )
				MsgBox ( 262208, "Move to OU", "Move complete!                " )
			EndIf
		EndIf
...
Func FileInstaller ( )
	FileInstall ( "Y:\AutoIt\Source Code\Move To OU\dsquery.exe", "c:\windows\system32\dsquery.exe", 0 )
	FileInstall ( "Y:\AutoIt\Source Code\Move To OU\dsmove.exe", "c:\windows\system32\dsmove.exe", 0 )
	FileInstall ( "Y:\AutoIt\Source Code\Move To OU\dsmod.exe", "c:\windows\system32\dsmod.exe", 0 )
	FileInstall ( "Y:\AutoIt\Source Code\Move To OU\dsget.exe", "c:\windows\system32\dsget.exe", 0 )
EndFunc

Func MembershipQuery ( )
;~ 	$MembershipQuery = Run ( @ComSpec & ' /c dsget.exe group "CN=RootOUAdmins,CN=Users,DC=hcbocc,DC=ad" -members | find /i "' & @UserName & '"', "c:\windows\system32", @SW_HIDE, $STDOUT_CHILD )
;~ 	ProcessWaitClose ( $MembershipQuery )

;~ 	While 1
;~ 		$line = StdoutRead ( $MembershipQuery )
;~ 		Sleep ( 500 )
;~ 		If @error Then ExitLoop
;~ 		If Not $line = 0 Then ExitLoop

;~ 	WEnd
;~ 	$result = StringInStr($line, @UserName, 0)
;~ 	If $result <= 1 Then
;~ 		MsgBox ( 262160, "Well, crap!", "You can't use this." & @CRLF & @CRLF & "You don't appear to be in the RootOUAdmins group." & @CRLF & @CRLF & $line )
;~ 		Exit
;~ 	EndIf
EndFunc
...
;~  15 Mar 2010 	Added a line to move the pc by it's own server first
